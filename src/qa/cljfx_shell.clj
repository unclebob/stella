...
(defn- run-connectors! []
  (with-app! {}
    (fn [^Stage stage]
      ...
      (when-not (ui/element-icon-label-equals? stage :converter "Converter1" "0")
        (fail! "Converter1 value 0 not shown on icon"))
      (pass! "connectors" "Converter1 placed with name and value 0")
...
(defn- run-edit-converter! []
  (with-app! {}
    (fn [^Stage stage]
      ...
      (when-not (ui/element-shows? stage :converter "Converter1" "Converter1")
        (fail! "Converter1 name not visible"))
      (ui/sync-converter-value-labels!)
      (when-not (ui/element-icon-label-equals? stage :converter "Converter1" "0")
        (fail! "Converter1 value 0 not shown on icon"))
      (ui/click-palette! stage "Connector")
      (ui/click-element! stage :converter "Converter1")
...
(defn- assert-converter-icon-value!
  [^Stage stage value & [message]]
  (ui/sync-converter-value-labels!)
  (when-not (ui/wait-for-element-icon-label! stage :converter "Converter1" value :attempts 50)
    (fail! (or message (str "Converter1 value not " value " on icon")))))

(defn- edit-converter-formula!
  [^Stage stage formula]
  (ui/right-click-element! stage :converter "Converter1")
  (when-not (ui/wait-for-dialog! "Edit Converter" :attempts 50)
    (fail! "Edit Converter dialog did not appear"))
  (ui/type-into-dialog-field! "Formula" formula)
  (ui/click-ok-on-dialog! "Edit Converter"))

(defn- run-converter-flow-rate! []
  (with-app! {}
    (fn [^Stage stage]
      (place-two-stocks! stage)
      (ui/click-palette! stage "Flow")
      (ui/click-element! stage :stock "Stock1")
      (ui/click-element! stage :stock "Stock2")
      (assert-flow! stage "Flow1" "Stock1" "Stock2"
                    :suite "converter-flow-rate" :show-name? true)
      (ui/click-palette! stage "Converter")
      (ui/click-in-region! stage :canvas [-50 80])
      (when-not (ui/wait-for-element! stage :converter "Converter1" :attempts 20)
        (fail! "Converter1 did not appear"))
      ...
      (ui/click-element! stage :stock "Stock1")
      (ui/click-element! stage :converter "Converter1")
      (when-not (ui/wait-for-element! stage :connector "Connector2" :attempts 20)
        (fail! "Connector2 did not appear"))
      (pass! "converter-flow-rate" "Connector2 links Stock1 to Converter1")
      (ui/click-palette! stage "Connector")
      (ui/click-element! stage :converter "Converter1")
      (ui/click-element! stage :flow "Flow1")
      (when-not (ui/wait-for-element! stage :connector "Connector1" :attempts 20)
        (fail! "Connector1 did not appear"))
      (assert-converter-icon-value! stage "0" "Converter1 value 0 not shown on icon")
      (pass! "converter-flow-rate" "Converter1 shows 0 after connectors placed")
      (edit-converter-formula! stage "Stock1 * 0.1")
      (ui/double-click-element! stage :stock "Stock1")
      (when-not (ui/wait-for-dialog! "Edit Stock" :attempts 50)
        (fail! "Edit Stock dialog did not appear"))
      (ui/type-into-dialog-field! "Initial value" "100")
      (ui/click-ok-on-dialog! "Edit Stock")
      (assert-converter-icon-value! stage "10" "Converter1 value not 10 after stock initial value set")
      (when-not (ui/element-shows? stage :flow "Flow1" "10")
        (fail! "Flow1 rate not 10"))
      (when-not (ui/connector-canvas-formula? "Connector1" "Stock1 * 0.1")
        (fail! "Connector1 formula not Stock1 * 0.1"))
      (when-not (ui/element-shows? stage :stock "Stock2" "0")
        (fail! "Stock2 value not 0 before stepping"))
      (pass! "converter-flow-rate" "Converter and flow show rate 10")
      (ui/click-step-button! stage)
      (ui/sync-converter-value-labels!)
      (when-not (ui/wait-for-stock-label! "Stock1" "99" :attempts 50)
        (fail! "Stock1 value not 99 after step"))
      (when-not (ui/wait-for-stock-label! "Stock2" "1" :attempts 50)
        (fail! "Stock2 value not 1 after step"))
      (assert-converter-icon-value! stage "9.9" "Converter1 value not 9.9 after step")
      (when-not (ui/wait-for-element-shows! stage :flow "Flow1" "9.9" :attempts 50)
        (fail! "Flow1 rate not 9.9 after step"))
      (pass! "converter-flow-rate" "Simulation transferred stock at computed rate")
      (edit-converter-formula! stage "(Stock1 + 10) / 2")
      (assert-converter-icon-value! stage "54.5")
      (pass! "converter-flow-rate" "Parenthesized formula evaluates to 54.5")
      (edit-converter-formula! stage "sqrt(Stock1)")
      (assert-converter-icon-value! stage "9.9" "sqrt(Stock1) not shown as 9.9")
      (pass! "converter-flow-rate" "sqrt function evaluates correctly")
      (ui/double-click-element! stage :stock "Stock1")
      (when-not (ui/wait-for-dialog! "Edit Stock" :attempts 50)
        (fail! "Edit Stock dialog did not reappear"))
      (ui/type-into-dialog-field! "Initial value" "3")
      (ui/click-ok-on-dialog! "Edit Stock")
      (edit-converter-formula! stage "Stock1 ^ 2")
      (assert-converter-icon-value! stage "9")
      (pass! "converter-flow-rate" "Power operator evaluates correctly")
      (edit-converter-formula! stage "foo(1)")
      (when-not (ui/wait-for-dialog! "Edit Converter" :attempts 20)
        (fail! "Edit Converter dialog closed after unknown function"))
      (ui/click-cancel-on-dialog! "Edit Converter")
      (when (ui/wait-for-dialog! "Edit Converter" :attempts 5)
        (fail! "Edit Converter dialog still visible after Cancel"))
      (pass! "converter-flow-rate" "Unknown function rejected; dialog dismissed on Cancel")
      (ui/quit-app! stage)
      (pass! "converter-flow-rate" "Quit requested"))))
...
(def ^:private suites
  {"qa-auto-close" run-qa-auto-close!
   ...
   "converter-flow-rate" run-converter-flow-rate!
