  ...
  (with-app! {}
    (fn [^Stage stage]
      ...
      (when-not (ui/wait-for-element! stage :connector "Connector1" :attempts 20)
        (fail! "Connector1 did not appear"))
      (ui/sync-converter-value-labels!)
      (when-not (ui/wait-for-element-icon-label! stage :converter "Converter1" "0")
        (fail! "Converter1 value 0 not shown on icon"))
      (pass! "converter-flow-rate" "Converter1 shows 0 after connector placed")
      (ui/right-click-element! stage :converter "Converter1")
      (when-not (ui/wait-for-dialog! "Edit Converter" :attempts 50)
        (fail! "Edit Converter dialog did not appear"))
      (ui/type-into-dialog-field! "Formula" "Stock1 * 0.1")
      ...
      (ui/click-ok-on-dialog! "Edit Stock")
      (ui/sync-converter-value-labels!)
      (when-not (ui/wait-for-element-icon-label! stage :converter "Converter1" "10")
        (fail! "Converter1 value not 10 after stock initial value set"))
      (when-not (ui/element-shows? stage :flow "Flow1" "10")
      ...
      (when-not (ui/wait-for-element-icon-label! stage :converter "Converter1" "9.9" :attempts 50)
        (fail! "Converter1 value not 9.9 after step"))
      ...
      (ui/right-click-element! stage :converter "Converter1")
      (when-not (ui/wait-for-dialog! "Edit Converter" :attempts 50)
        (fail! "Edit Converter dialog did not reappear"))
      (ui/type-into-dialog-field! "Formula" "Missing * 2")
      (ui/click-ok-on-dialog! "Edit Converter")
      (when-not (ui/wait-for-dialog! "Edit Converter" :attempts 20)
        (fail! "Edit Converter dialog closed after invalid formula"))
      ...
      (pass! "converter-flow-rate" "Invalid formula rejected; dialog dismissed on Cancel")
      (ui/quit-app! stage)
